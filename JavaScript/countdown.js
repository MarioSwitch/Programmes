...
Usage:
    ...
    Set id to "countdown" for the element you want to transform into a countdown.
...
Utilisation :
    ...
    Mettez l'identifiant de l'élément que vous souhaitez transformer en compte à rebours sur "countdown".
*/

function countdownTo(end, format = "%countdown", endMessage = ""){
    ...
    if(remainingTime<360000000 && remainingTime>=6000000){ //if between 100 mins (1h 40m) and 100 hours (4d 4h)
        var remainingString = Math.floor(remainingTime/3600000)+"h "+Math.floor(remainingTime/60000)%60+"m";
    }
    if(remainingTime<6000000 && remainingTime>=60000){
        var remainingString = Math.floor(remainingTime/60000)+"' "+Math.floor(remainingTime/1000)%60+"''";
    }
    ...
    setTimeout(countdownTo, 1000, end, format, endMessage);
